...
#### 创建人物
 - 第一行 `local 梦缘 = GalGame.引擎:创建人物('梦旅')` 是创建一个人物，用于在剧情中使用。  
    你可以根据需要创建多个人物，用于在剧情中使用。
 - 由于剧情脚本采用面向对象，创建人物都需要使用 `GalGame.引擎:创建人物('人物名称')` 方法，  
...
### 基础语法
 ...
 - 很多方法都是用`GalGame`这个对象来调用的，例如：`GalGame.引擎:显示背景('图片/背景/教室背景.png')` 来显示背景图片。

### 显示背景图片
``` lua
local 梦缘 = GalGame.引擎:创建人物('梦缘')
...
```
 - 你可以使用 `GalGame.引擎:显示背景('背景图片路径')` 方法来显示游戏背景图片。
...
```
 - 你可以使用 `人物对象:添加立绘('立绘名称','图片路径')` 方法来添加人物立绘。
    其中 `立绘名称` 是你为立绘指定的名称，用于在剧情中引用立绘。
    `图片路径` 是你为立绘指定的路径，用于在剧情中引用立绘图片。
 - 推荐将图片放在 **图片/立绘** 目录下，方便管理。
 - 你可以使用 `人物对象:显示立绘('立绘名称')` 方法来显示人物立绘。
    其中 `立绘名称` 是你为立绘指定的名称，用于在剧情中引用立绘。  
...
return function()
    ...
    GalGame.引擎:显示背景('图片/背景/走廊背景2.png'):转场('向左推入 2.0')
    梦女:说('当前测试：向左推入转场')

    GalGame.引擎:显示背景('图片/背景/学校操场.png'):转场('向右推入 2.0')
    梦女:说('当前测试：向右推入转场')

    GalGame.引擎:显示背景('图片/背景/走廊背景1.png'):转场('向下推入 2.0')
    梦女:说('当前测试：向下推入转场')

    GalGame.引擎:显示背景('图片/背景/教室背景.png'):转场('向上推入 2.0')
    梦女:说('当前测试：向上推入转场')
    return
end
```
...
- `GalGame.引擎:转场命令('屏幕','淡出 1.0')` 是直接使用Galgame对象来进行转场，  
  其中的 `屏幕` 是转场的目标，转场的目标有三个，分别是**屏幕、前景、背景**，这里是屏幕  
  其中的 `淡出 1.0` 是转场的类型和时间，这里是淡出效果，时间是1秒  
- `GalGame.引擎:转场命令('屏幕','淡入 1.0')` 会在淡出转场完成后执行
- `GalGame.引擎:显示背景('图片/背景/走廊背景2.png')` 是转场作用的目标背景
- `GalGame.引擎:等待(2.0)` 会等待2秒，确保转场完成后再执行后续代码
...
return function()
    ...
    GalGame.引擎:图片转场命令('背景','图片/转场/WIP_BRTL.png','2.0')
    GalGame.引擎:显示背景('图片/背景/学校操场.png')
    梦女:说('当前转场测试：图片/转场/WIP_BLTR')
    return
end
```
- 使用 `GalGame.引擎:图片转场命令('背景','图片/转场/WIP_BLTR.png','2.0')` 来自定义图片转场。  
  其中的 `背景` 是转场的目标 
...
return function()
 ...
 ```
 - 使用 `GalGame.引擎:显示前景('图片/背景/学校操场.png'):转场('溶解 2.0')` 可以在前景层显示图片并进行溶解转场。
 - 使用 `GalGame.引擎:显示屏幕('图片/立绘/女孩1.png')` 可以在屏幕前显示立绘。
...
return function()
    ...
    GalGame.引擎:转场命令('背景','向上推入 2.0')
    GalGame.引擎:显示背景('图片/背景/走廊背景2.png')
    梦女:说('当前转场测试：在背景层进行转场，不影响立绘')

    GalGame.引擎:转场命令('前景','向左推入 2.0')
    GalGame.引擎:显示背景('图片/背景/学校操场.png')
    梦女:说('当前转场测试：在前景层进行转场，影响立绘')

    GalGame.引擎:转场命令('屏幕','向右推入 2.0')
    GalGame.引擎:显示背景('图片/背景/学校操场.png')
    梦女:说('当前转场测试：在屏幕层使用转场图片进行转场，影响全屏')
end
  ```
  - 使用 `GalGame.引擎:转场命令('背景','向上推入 2.0')` 可以在背景层进行向上推入转场。
  - 使用 `GalGame.引擎:转场命令('前景','向左推入 2.0')` 可以在前景层进行向左推入转场。
  - 使用 `GalGame.引擎:转场命令('屏幕','向右推入 2.0')` 可以在屏幕层使用转场图片进行向右推入转场。
...
return function()
 ...
 - 使用 `GalGame.引擎:显示背景(颜色)` 和 `GalGame.引擎:显示前景(颜色)` 以及 `GalGame.引擎:显示屏幕(颜色)` 可以在不同图层上显示不同的颜色。
